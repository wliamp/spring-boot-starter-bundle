...
class ZaloParty(
    ...
    override fun verify(token: String): Mono<Boolean> {
        if (props.zaloTokenInfoUrl.isBlank() || props.zaloAppId.isBlank()) {
            return Mono.error(IllegalStateException("Zalo configuration missing"))
        }
        val uri = "${props.zaloTokenInfoUrl}?access_token=$token"
    ...
    override fun getInfo(token: String): Mono<Map<String, Any>> {
        ...
        val uri = "${props.zaloTokenInfoUrl}?access_token=$token&fields=${props.zaloInfoFields}"
        return webClient.get()
            .uri(uri)
            .retrieve()
            .onStatus({ it.isError }) { response ->
